...
export default async function ObraPage({ params }: { params: { obraId: string } }) {
  ...
  if (!obra) return <div style={{ color: 'var(--text-primary)', padding: 40 }}>Obra no encontrada</div>
    ...
    <main style={{ minHeight: '100vh', background: 'var(--bg-main)', color: 'var(--text-primary)', fontFamily: 'system-ui, sans-serif', padding: '40px' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
          <Link href="/" style={{ color: 'var(--text-muted)', fontSize: 13, textDecoration: 'none' }}>← Volver a obras</Link>
          <ThemeToggle />
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
          <div>
            <h1 style={{ fontSize: 26, fontWeight: 700, color: 'var(--accent)', marginBottom: 4 }}>{obra.nombre}</h1>
            <p style={{ color: 'var(--text-muted)', fontSize: 14 }}>{obra.direccion}</p>
          </div>
          <span style={{ background: 'var(--success-bg)', color: 'var(--success)', padding: '4px 12px', borderRadius: 20, fontSize: 12 }}>● {obra.estado}</span>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14, marginBottom: 28 }}>
          {[
            ...
            { label: 'Deuda proveedores', valor: saldoProveedores > 0 ? `$ ${fmt(saldoProveedores)}` : '✓ Al día', color: saldoProveedores > 0 ? '#F87171' : 'var(--success)', sub: 'Saldo base sin CAC/IVA' },
            { label: 'Cuotas pendientes', valor: cuotasPendientes.length, color: cuotasPendientes.length > 0 ? '#F59E0B' : 'var(--success)', sub: `$ ${fmt(totalCuotasArs)} por cobrar` },
            { label: 'Cuotas vencidas', valor: cuotasVencidas.length, color: cuotasVencidas.length > 0 ? '#F87171' : 'var(--success)', sub: cuotasVencidas.length > 0 ? 'Requieren atención' : 'Todo al día' },
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14, marginBottom: 32 }}>
          {modulos.map(m => (
            <Link key={m.label} href={m.href} style={{ textDecoration: 'none' }}>
              <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 10, padding: '18px 24px', display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {contratos?.map((contrato: any) => (
                <tr key={contrato.id} style={{ borderTop: '1px solid var(--border)' }}>
                  <td style={{ padding: '12px 16px', fontWeight: 500 }}>
                    <Link href={`/obras/${obraId}/clientes/${contrato.id}`} style={{ color: 'var(--text-primary)', textDecoration: 'none' }}>
                  ...
                  <td style={{ padding: '12px 16px', fontFamily: 'monospace', color: 'var(--text-secondary)', fontSize: 13 }}>$ {fmt(contrato.tc_contrato)}</td>
                  <td style={{ padding: '12px 16px', color: 'var(--text-secondary)', fontSize: 13 }}>{contrato.n_cuotas} cuotas</td>
                  <td style={{ padding: '12px 16px' }}>
                    <span style={{ background: 'var(--success-bg)', color: 'var(--success)', padding: '2px 8px', borderRadius: 20, fontSize: 11 }}>{contrato.estado}</span>
                  </td>
                </tr>
              ))}
            </tbody>
