      ...
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28 }}>
          <div>
            ...
            <p style={{ color: '#556070', fontSize: 14 }}>Rubros y seguimiento de ejecución · Guatemala 5934</p>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14, marginBottom: 28 }}>
          {[
            ...
            { label: 'Rubros nivel 1', valor: rubrosNivel1.length, color: '#E8EDF5' },
        ...
        <div style={{ background: '#161B25', border: '1px solid #252D3D', borderRadius: 10, overflow: 'hidden' }}>
          <div style={{ padding: '14px 20px', borderBottom: '1px solid #252D3D', fontSize: 13, fontWeight: 600 }}>
            ...
            <thead>
              <tr style={{ background: '#1D2535' }}>
                {['Código', 'Descripción', 'Ítems', 'Total presupuestado', 'Acciones'].map(h => (
                  <th key={h} style={{ padding: '9px 16px', textAlign: 'left', fontSize: 11, color: '#556070', fontWeight: 500, textTransform: 'uppercase', letterSpacing: 0.5 }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {rubrosNivel1.length === 0 ? (
                ...
                return (
                  <>
                    <tr key={r.id} style={{ borderTop: '1px solid #252D3D', background: 'rgba(255,255,255,0.02)' }}>
                      ...
                      <td style={{ padding: '12px 16px', color: '#8A96AA', fontSize: 13 }}>{itemsRubro.length}</td>
                    ...
                    {hijosRubro.map(h => {
                      ...
                      const totalHijo = itemsHijo.reduce((a, i) =>
                        a + ((i.cantidad || 0) * ((i.costo_unit_mat || 0) + (i.costo_unit_mo || 0))), 0)
                      return (
                        <tr key={h.id} style={{ borderTop: '1px solid #252D3D' }}>
                          <td style={{ padding: '10px 16px 10px 32px', fontFamily: 'monospace', color: '#556070', fontSize: 12 }}>{h.codigo}</td>
                          <td style={{ padding: '10px 16px 10px 32px', color: '#8A96AA', fontSize: 13 }}>{h.descripcion}</td>
                          <td style={{ padding: '10px 16px', color: '#556070', fontSize: 12 }}>{itemsHijo.length}</td>
                          ...
                          <td style={{ padding: '10px 16px' }}>
                            <Link href={`/obras/${obraId}/presupuesto/${h.id}`}
                              style={{ color: '#556070', fontSize: 12, textDecoration: 'none' }}>
                              Ver →
                            </Link>
                          </td>
                        </tr>
